...
## 数据集（dataset）：BITFlyDet

BITFlyDet是一个专为无监督跨域目标检测任务设计的数据集，分为源域数据集和目标域数据集两部分。源域数据集包含3162张精细标注的图像，不仅包含多高度、视角的复杂航拍场景图像，还为每张图像提供了对应的传感器元数据，包括拍摄参数、飞行高度、拍摄视角、GPS坐标及拍摄时间。数据集共包含21,024张高质量真实航拍图像，为服务于不同算法的训练与评估需求。

**数据集组成**

BITFlyDet数据集共标注了四种类别：汽车、公共汽车、面包车和卡车，共221743个实例注释。它还为每张图像提供了对应的传感器元数据，包括拍摄参数、飞行高度、拍摄视角、GPS坐标及拍摄时间。训练集：17,716张图像，用于模型训练；评估/测试集：3,308张图像，用于公正评估算法性能。数据可依据元数据（如高度、视角、天气）灵活划分为不同的“源域”与“目标域”，以支持算法所研究的在特定域偏移下的迁移能力评估。图像数据本身具备时序连续性，可构建无标注视频流，以验证算法利用时间一致性进行自监督适应的能力。严格按照时序与场景连续性进行采集与组织。所有图像均同步记录了12个维度的机载传感器元数据。

![BITFlyDet](assets/images/4.jpg)

图 1：BITFlyDet数据集

**主要特性**
- **多样的采集条件**：BITFlyDet中的图像由无人机在10米到200米左右的不同高度拍摄，相机的视角在0度到-90度俯仰角范围变化。
- **丰富的传感器数据**：该数据集的每张图像均提供了5类传感器元数据，包括拍摄参数、飞行高度、拍摄视角、GPS坐标及拍摄时间。
- **跨域数据**：数据集分成源域数据和目标域数据两部分，目标域仅包含拥有传感器元数据的无标注图像，为跨域迁移的泛化能力提供衡量基准。

...
BITFlyDet是一个专为无监督跨域目标检测任务设计的数据集，分为源域数据集和目标域数据集两部分。源域数据集包含3162张精细标注的图像，不仅包含多高度、视角的复杂航拍场景图像，还为每张图像提供了对应的传感器元数据，包括拍摄参数、飞行高度、拍摄视角、GPS坐标及拍摄时间。为了模拟真实的低成本部署场景，目标域数据集提供了超过3万张涵盖多源环境的无标注图。不同于以往数据集，这3万张图像均保留了完整的传感器元数据，旨在探索一种利用传感器元数据弥补领域偏移的无监督跨域迁移范式。

**数据集组成**:

BITFlyDet数据集包含3162张精细标注的图像，共标注了四种类别：汽车、公共汽车、面包车和卡车，共221743个实例注释。它还为每张图像提供了对应的传感器元数据，包括拍摄参数、飞行高度、拍摄视角、GPS坐标及拍摄时间。目标域数据集还提供了超过3万张涵盖多源环境的无标注图。

**主要特性**
- **多样的采集条件**：BITFlyDet中的图像由无人机在90米到350米左右的不同高度拍摄，相机的视角在0度到-90度俯仰角范围变化。
